...
export function EpisodeModal({ episode, onClose }: EpisodeModalProps) {
  const { addEpisode, updateEpisode } = useEpisodes();
  ...
  useEffect(() => {
    if (episode) {
      setFormData({
        ...
        due_date: episode.due_date ? format(new Date(episode.due_date), 'yyyy-MM-dd') : '',
        guest_name: episode.guest_name || '',
        recording_date: episode.recording_date ? format(new Date(episode.recording_date), 'yyyy-MM-dd') : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (episode) {
        ...
        await addEpisode(data as NewEpisode);
      }
      onClose();
    } catch (error) {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {formData.episode_type === 'interview' && (
            <>
              <div className="border-t pt-4">
                <h3 className="text-sm font-medium text-gray-700 mb-3">インタビュー情報</h3>
          ...
          {formData.episode_type === 'vtr' && (
            <div className="border-t pt-4">
              <h3 className="text-sm font-medium text-gray-700 mb-3">VTR情報</h3>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  素材準備状況
                </label>
