...
export default function ProgramList() {
    ...
    return programs
      ...
      .filter(program => {
        if (!filters.showPastPrograms && program.first_air_date) {
          const airDate = new Date(program.first_air_date + 'T00:00:00');
  ...
  const futureReAirings = React.useMemo(() => {
    ...
    return programs
      .filter(program => 
        program.re_air_date && 
        new Date(program.re_air_date + 'T00:00:00') >= today
      )
      .sort((a, b) => {
      ...
      <div className="space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <h2 className="text-xl md:text-2xl font-semibold text-text-primary">エピソード一覧</h2>
            {hasActiveFilters && (
